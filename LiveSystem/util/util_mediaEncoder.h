...
public:
  ...
  int nbSampleIn = 1024; // number of samples in the input XData
  ...
  //输出参数
    ...
    eprintf("init resample format, %d, %d", outSampleFmt, inSampleFmt);
    ...
    eprintf("peek data in input data: [");
    for (int i = 0; i < 10; ++i) {
      eprintf("%f, %f", *((float *)(d.data + i * 8)),
              *((float *)(d.data + i * 8 + 4)));
    }
    eprintf("]");
    ...
    eprintf("peek data in pcm->data: [");
    for(int i = 0;i < 10; ++i) {
      eprintf("%f, %f", *((float *)(pcm->data[0] + i * 4)),*((float *)(pcm->data[1] + i * 4)));
    }
    eprintf("]");
		pcm->pts = d.pts;
		r.data = (char*)pcm;
		r.size = sizeof(AVFrame);
		r.pts = d.pts;
		return r;
	}
    ...
    eprintf("p->pts: %ld, p->nb_samples: %d, p->sample_rate: %d", p->pts, p->nb_samples, p->sample_rate);
		...
		if (ret != 0) {
      fprintf(stderr, "send audio frame to encoder failed: %d\n", ret);
      char err[1024] = {0};
      ...
      return r;
		...
		if (ret != 0) {
      fprintf(stderr, "receive audio packet from encoder failed: %d\n", ret);
      char err[1024] = {0};
      av_strerror(ret, err, 1024);
      fprintf(stderr, "reason: %s\n", err);
      return r;
